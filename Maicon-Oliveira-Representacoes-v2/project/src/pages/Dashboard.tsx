...
const KpiCard: React.FC<{ kpi: KpiData, onClick?: () => void }> = ({ kpi, onClick }) => {
  ...
  return (
    <div 
      onClick={onClick}
      className={cn(
        "bg-white p-4 rounded-2xl border border-neutral-200 shadow-sm hover:shadow-md transition-all",
        onClick && "cursor-pointer hover:border-orange-300 active:scale-[0.98]"
      )}
    >
      ...
      <p className="text-[10px] font-semibold text-neutral-400 uppercase tracking-widest mb-1">{kpi.label}</p>
      <h3 className="text-xl font-black text-neutral-900 truncate leading-tight">{kpi.format(kpi.value)}</h3>
      <p className="text-[11px] text-neutral-400 mt-1">vs {kpi.format(kpi.previousValue)}</p>
    </div>
  );
};

const ChartCard: React.FC<{ title: string, children: React.ReactNode, className?: string }> = ({ title, children, className }) => (
  <div className={cn("bg-white p-4 rounded-2xl border border-neutral-200 shadow-sm flex flex-col h-full", className)}>
    <h3 className="text-xs font-bold text-neutral-500 mb-4 uppercase tracking-widest flex items-center gap-2 shrink-0">
